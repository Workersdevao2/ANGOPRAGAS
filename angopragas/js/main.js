...
(function () {
  ...
  /* Force muted autoplay (Safari / iOS / Chrome policies) */
  function forceVideoPlay(video) {
    ...
    const tryPlay = () => {
      ...
      if (p && typeof p.then === 'function') {
        p.catch(() => {
          /* Retry after short delay — common on Safari */
          setTimeout(() => {
            video.muted = true;
            video.play().catch(() => {});
          }, 200);
  ...
  function setupAutoplayVideos() {
    const videos = $$('video.js-autoplay');
    videos.forEach(forceVideoPlay);

    /* Resume when tab becomes visible again */
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible') {
        videos.forEach(v => {
          if (v.paused) forceVideoPlay(v);
        });
      }
    });
